    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-6 border-l-4" style={{ borderColor: COLORS.Work }}>
          ...
          <p className="text-3xl font-bold" style={{ color: COLORS.Work }}>{totalHours.Work.toFixed(1)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4" style={{ borderColor: COLORS.Study }}>
          ...
          <p className="text-3xl font-bold" style={{ color: COLORS.Study }}>{totalHours.Study.toFixed(1)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4" style={{ borderColor: COLORS.Life }}>
          ...
          <p className="text-3xl font-bold" style={{ color: COLORS.Life }}>{totalHours.Life.toFixed(1)}</p>
        </div>
      </div>
              ...
              <div key={item.category} className={index < getRanking().length - 1 ? 'pb-3 border-b border-gray-200' : ''}>
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-lg font-medium" style={{ color: item.color }}>{item.category}</span>
                  </div>
                  <span className="text-lg font-bold text-gray-900">{item.hours} hrs</span>
                </div>
                <p className="text-sm text-gray-500 mt-1">
                  {((parseFloat(item.hours) / totalSum) * 100).toFixed(1)}% of total time
                </p>
              </div>
            ))}
          </div>
        ...
        {['Work', 'Study', 'Life'].map((category) => (
          <div key={category} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2">
              ...
              {getSubcategoryRanking(category).length === 0 && (
                <p className="text-sm text-gray-500">No events in this category</p>
              )}
            </div>
          </div>
        ))}
