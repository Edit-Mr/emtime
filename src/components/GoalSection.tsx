...
const GoalSection: React.FC<GoalSectionProps> = ({ goalAnalysis }) => {
  ...
  const calculateSummary = (
    data: GoalAnalysis[],
    actualKey: 'codingHours' | 'studyHours' | 'sportHours',
    goalKey: 'codingGoal' | 'studyGoal' | 'sportGoal'
  ) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-600">{codingSummary.daysMetGoal} / {codingSummary.totalDays} days</span>
              <span className="font-semibold text-gray-900">{codingSummary.percentage}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-primary-600 h-2 rounded-full transition-all duration-500"
                style={{ width: `${codingSummary.percentage}%` }}
              ></div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-600">{studySummary.daysMetGoal} / {studySummary.totalDays} days</span>
              <span className="font-semibold text-gray-900">{studySummary.percentage}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-green-500 h-2 rounded-full transition-all duration-500"
                style={{ width: `${studySummary.percentage}%` }}
              ></div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-600">{sportSummary.daysMetGoal} / {sportSummary.totalDays} days</span>
              <span className="font-semibold text-gray-900">{sportSummary.percentage}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-yellow-500 h-2 rounded-full transition-all duration-500"
                style={{ width: `${sportSummary.percentage}%` }}
              ></div>
            </div>
          </div>
