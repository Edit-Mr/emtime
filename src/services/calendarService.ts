...
export const fetchCalendarEvents = async (
  ...
  try {
    ...
    const targetCalendars = calendars.filter((cal: any) =>
      calendarNames.some(name => cal.summary?.toLowerCase().includes(name.toLowerCase()))
    );
...
export const isFamilyDay = (event: CalendarEvent): boolean => {
  ...
  return (summary.includes('family') || description.includes('family')) && 
         !!event.start.date; // whole day event
};
